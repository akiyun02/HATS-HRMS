...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 flex justify-between items-end">
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold leading-tight text-slate-900 dark:text-white sm:text-2xl">Edit Policy</h2>
            </div>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400 ml-8">Update rules for {{ $leavePolicy->name }}.</p>
        </div>
    ...
    <form action="{{ route('leave-policies.update', $leavePolicy) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        <div class="bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 rounded-lg p-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name', $leavePolicy->name) }}" class="form-input w-full bg-slate-50/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 focus:ring-brand-600 focus:border-brand-600 rounded-lg text-sm font-semibold">
                </div>
                <div class="flex items-end gap-8 pb-3">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_default" value="1" {{ $leavePolicy->is_default ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-brand-600 focus:ring-brand-600 dark:bg-slate-800 transition-all">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_probationary" value="1" {{ $leavePolicy->is_probationary ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-brand-600 focus:ring-brand-600 dark:bg-slate-800 transition-all">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="form-input w-full bg-slate-50/50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 focus:ring-brand-600 focus:border-brand-600 rounded-lg text-sm font-medium">{{ old('description', $leavePolicy->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden">
            <div class="px-8 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
                ...
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Modify days and accrual types</span>
            </div>
            <div class="p-0 overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        ...
                        @php
                            $pivot = $leavePolicy->leaveTypes->where('id', $type->id)->first()?->pivot;
                        @endphp
                        <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                            ...
                            <td class="px-8 py-5 text-center">
                                <input type="number" name="leave_types[{{ $index }}][annual_days]" step="0.5" min="0" value="{{ old("leave_types.$index.annual_days", $pivot?->annual_days ?? 0) }}" class="form-input w-20 text-center text-sm font-bold bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-md focus:ring-brand-500">
                            </td>
                            <td class="px-8 py-5 text-center">
                                <select name="leave_types[{{ $index }}][accrual_type]" class="form-input text-xs font-bold bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-md focus:ring-brand-500 uppercase tracking-tight">
                                    <option value="fixed" {{ (old("leave_types.$index.accrual_type", $pivot?->accrual_type) == 'fixed') ? 'selected' : '' }}>Fixed (Lump Sum)</option>
                                    <option value="prorated" {{ (old("leave_types.$index.accrual_type", $pivot?->accrual_type) == 'prorated') ? 'selected' : '' }}>Prorated (By Hire Date)</option>
                                    <option value="monthly" {{ (old("leave_types.$index.accrual_type", $pivot?->accrual_type) == 'monthly') ? 'selected' : '' }}>Monthly (Accrual)</option>
                                </select>
                            </td>
                            <td class="px-8 py-5 text-center">
                                <input type="number" name="leave_types[{{ $index }}][carry_over_limit]" step="0.5" min="0" value="{{ old("leave_types.$index.carry_over_limit", $pivot?->carry_over_limit ?? 0) }}" class="form-input w-20 text-center text-sm font-bold bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-md focus:ring-brand-500">
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
